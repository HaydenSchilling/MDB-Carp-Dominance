...
WRPA_all <-st_read("../../database exploration/WRPA March2023/WRPA edit March2023.shp") %>%
...
all_sites <- read_csv("Data/2024 full with lats lons.csv")

## now other sites


elevation_dat <- read_csv("Data/Site elevations.csv")
#all_data <- all_data 


catch <- read_csv("Data/clean boat and backpack electro catch with waterbody.csv") %>% 
...
data_summaryX <- catch %>% group_by(WaterbodyName, coords.x1, coords.x2) %>%
  summarise(n_survey = n_distinct(SamplingRecordID)) %>% 
  mutate(WaterbodyName = case_when(WaterbodyName == "Darling River" ~ "Darling-Baaka River",
                                   T ~ WaterbodyName))

data_summaryX2 <- all_sites %>% rename(coords.x1 = SampleLongitude, coords.x2 = SampleLatitude) %>%
  group_by(coords.x1, coords.x2) %>% anti_join(data_summaryX) %>%
  summarise(n_survey = n_distinct(SamplingRecordID))

### Site Map
p1 <- ggplot(data_summaryX, aes(coords.x1, coords.x2, col = WaterbodyName, size=n_survey)) +
  ...
  geom_point(data=data_summaryX2, aes(coords.x1, coords.x2, size=n_survey), 
             inherit.aes = F, col="black", fill="white", shape=21)+
...
ggsave("Site map2024.png", dpi = 600, width=21, height=21, units="cm")
ggsave("Site map2024.pdf", dpi = 600, width=21, height=21, units="cm")
