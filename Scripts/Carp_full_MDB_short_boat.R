...
elevation_dat <- read_csv("Site elevations.csv") %>% rename(SampleLongitude = coords.x1, SampleLatitude = coords.x2)
...
ab <- read_csv("a b values.csv")

bio <- read_csv("All bio_11_08_2023.csv")  %>% left_join(ab) %>%
...
f2 <- brm(Response ~ fYear + (1|FDate) + (1|SiteID) + (1|SWWRPANAME_NEW), #  (1|MethodType)
          ...
          cores=4,
          file_refit = "always",
          file = "Carp full MDB WRPA day site random boat only.rds")
